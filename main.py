...
class MyWidget(QtWidgets.QLabel):
    ...
    def color(self, x, y):
        if self.fractal_type == 'mandelbrot':
            ...
            return self.color_biomorph(x, y)
    ...
    def color_biomorph(self, x, y):
        ...
        while abs(z) < 2 and iteration < max_iter:
            z = z*z + self.const
            iteration += 1
        color_value = int((iteration/max_iter) * 255)
        if iteration < max_iter:
            return (color_value << 16) + ((color_value + 30) % 256 << 8) + (color_value + 68) % 256
        else:
            return 0x000000
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def init_menu(self):
        ...
        action_biomorph = QAction("Биоморф", self, checkable=True)
        action_biomorph.setChecked(self.viewer.fractal_type == "biomorph")
        action_biomorph.triggered.connect(lambda: self.viewer.set_fractal_type("biomorph"))
        fractal_group.addAction(action_biomorph)
        fractal_menu.addAction(action_biomorph)        
